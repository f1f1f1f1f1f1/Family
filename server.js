...
function proxyToHA(req, res) {
  ...
  // Signed media URLs (/media/local/...?authSig=...) are self-authenticating
  // by design — the signature alone is meant to grant access with no
  // separate auth header (e.g. so Chromecast/media players can fetch them
  // directly). Stacking an unrelated Bearer token on top of an already-
  // signed request can cause HA to reject it even though the signature is
  // valid, so we omit auth entirely for /media/* and rely on authSig.
  const isSignedMediaRequest = req.url.startsWith('/media/');
  ...
  req.on('end', () => {
    ...
    const options = {
      ...
      method: req.method,
      headers: {
        ...(isSignedMediaRequest ? {} : { 'Authorization': `Bearer ${SUPERVISOR_TOKEN}` }),
